...
# ==========================================
# Kita ubah Target menjadi Logaritma. Ini memaksa Regresi Linier 
# untuk memodelkan pertumbuhan Eksponensial (Bunga Majemuk) dengan sangat mulus!
...
# ==========================================
# 5. THE LOG-LINEAR ENSEMBLE
# ==========================================
print("\nMelatih Model Rekursif (The Log-Transformed Actuary)...")
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for log_target in log_targets:
        ...
        y_train = train_df[log_target] # TARGETNYA ADALAH LOGARITMA
        
        X_test = temp_df[temp_df['Date'] == pred_date][features].fillna(0)
        
        # 3 Mesin Linear Paling Tahan Banting di Scikit-Learn
        ...
        # BLEND EMAS (40/30/30) PADA ALAM LOGARITMA
        final_log_val = (0.40 * pred_bayes) + (0.30 * pred_ridge) + (0.30 * pred_huber)
        
        # Simpan nilai log untuk iterasi bulan depan (sebagai Lag)
        current_ts_data.loc[current_ts_data['Date'] == pred_date, log_target] = final_log_val
        
        # REVERSE LOG: Mengubahnya kembali menjadi angka asli (Eksponensial)
        actual_val = np.expm1(final_log_val)
        
        # Simpan nilai asli ke kolom aslinya dan ke dictionary output
        if log_target == 'Log_Freq':
            ...
            actual_val = actual_val * 1e9 # Kembalikan miliaran
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE LOG-TRANSFORMED ACTUARY) ---")
...
submission_df.to_csv('submission_log_actuary.csv', index=False)
print("\n[LOCKED] File 'submission_log_actuary.csv' siap!")
print("Transformasi Log-Linear telah dieksekusi! Kurva eksponensial ini siap menjebol < 3.0!")