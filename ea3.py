...
def create_features(df):
    ...
    for col in ['Claim_Frequency', 'Total_Claim_B']:
        # KEMBALI HANYA GUNAKAN LAG 1, 2, 3, 6 (Tanpa 12 yang merusak!)
        for i in [1, 2, 3, 6]:
            df[f'{col}_lag_{i}'] = df[col].shift(i)
        
        # HANYA gunakan Roll Mean (Tanpa EWMA yang bikin fluktuatif)
...
# ==========================================
# 3. RECURSIVE ENSEMBLE TRAINING (THE BAYESIAN SUPREMACY)
# ==========================================
print("\nMelatih Model Recursive (The 2.9 Absolute Form)...")
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        ...
        # THE ULTIMATE BLEND: Holt-Winters diturunkan drastis! Bayesian Ridge adalah MVP kita.
        final_val = (0.60 * pred_bayes) + (0.25 * pred_ridge) + (0.15 * pred_hw)
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE 2.9 ABSOLUTE) ---")
...
# Nama file diubah untuk memastikan tidak tertukar
submission_df.to_csv('submission_the_2_9_absolute.csv', index=False)
print("\n[LOCKED] File 'submission_the_2_9_absolute.csv' siap!")
print("Holt-Winters direm, Bayesian Ridge dipacu maksimal! Hajar batas < 3.0 itu bro!")