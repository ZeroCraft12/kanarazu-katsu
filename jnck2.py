...
# ==========================================
# Tidak pakai log, tidak pakai filter rumit. Kembali ke skala asli.
...
# Capping 98.0% (Fondasi terkuat penstabil skor 3.6)
...
# ==========================================
# 3. FITUR KINEMATIK (AUTOREGRESSIVE)
# ==========================================
...
# ==========================================
# 4. THE MULTIPLICATIVE PROPHET ENSEMBLE
# ==========================================
print("\nMelatih Model Rekursif (The Multiplicative Prophet)...")
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        # DATA LATIHAN (Selalu diperbarui setiap iterasi bulan)
        train_df = temp_df[temp_df['Date'] < pred_date].dropna().copy()
        
        # -------------------------------------------------------------------
        # ALGORITMA 1: MULTIPLICATIVE BASELINE (Kunci Penembus 3.0!)
        # -------------------------------------------------------------------
        # A. Ekstrak Tren Utama dengan Huber
        train_df['Time_Index_Temp'] = np.arange(1, len(train_df) + 1)
        trend_model = HuberRegressor().fit(train_df[['Time_Index_Temp']], train_df[target])
        train_df['Trend_Base'] = trend_model.predict(train_df[['Time_Index_Temp']])
        
        # B. Ekstrak Rata-Rata Musiman sebagai RASIO (Bukan Selisih)
        # Mencegah error pembagian dengan 0 menggunakan batas bawah epsilon
        train_df['Seasonality_Ratio'] = train_df[target] / (train_df['Trend_Base'] + 1e-6)
        seasonality_map = train_df.groupby('Month')['Seasonality_Ratio'].mean().to_dict()
        
        # C. Tebak Baseline Masa Depan secara Multiplikatif
        future_time_idx = len(train_df) + 1
        future_trend = trend_model.predict(np.array([[future_time_idx]]))[0]
        future_month = pd.to_datetime(pred_date).month
        # Default ratio adalah 1.0 (netral) jika bulan tidak ditemukan
        future_seasonality_ratio = seasonality_map.get(future_month, 1.0)
        
        # DIKALI, bukan DITAMBAH! Lonjakan akhir tahun ikut menggelembung proporsional
        pred_baseline = future_trend * future_seasonality_ratio
        # -------------------------------------------------------------------
        
        # ALGORITMA 2 & 3: PENANGKAP MOMENTUM (BayesianRidge & Ridge)
        ...
        # THE GRAND BLEND: Karena Baseline kita sekarang SUPER CERDAS, kita naikkan bobotnya!
        # 40% Multiplicative Prophet + 30% Bayes + 30% Ridge
        final_val = (0.40 * pred_baseline) + (0.30 * pred_bayes) + (0.30 * pred_ridge)
        
        current_ts_data.loc[current_ts_data['Date'] == pred_date, target] = final_val
        
        # Kembalikan Skala Miliaran
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE MULTIPLICATIVE PROPHET) ---")
...
submission_df.to_csv('submission_multiplicative_prophet.csv', index=False)
print("\n[LOCKED] File 'submission_multiplicative_prophet.csv' siap!")
print("Musiman Multiplikatif AKTIF! AI kini mengalikan tren masa depan secara proporsional. GAS!")