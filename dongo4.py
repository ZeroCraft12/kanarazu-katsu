...
# ==========================================
# 1. PONDASI DATA (ERA NORMAL BARU)
# ==========================================
...
# ==========================================
# 2. ISOLATED AUTOREGRESSIVE FEATURES 
# ==========================================
def create_features(df):
    ...
    for col in ['Claim_Frequency', 'Total_Claim_B']:
        # KUNCI PENEMBUS 3.0: KITA TAMBAHKAN LAG 12
        for i in [1, 2, 3, 6, 12]:
...
# ==========================================
# 3. ENSEMBLE TRAINING (THE 2023 ISOLATION)
# ==========================================
print("\nMelatih Model Recursive (The 2023 Clean Isolation)...")
current_ts_data = ts_data.copy()
final_preds = {'2025_08': {}, '2025_09': {}, '2025_10': {}, '2025_11': {}, '2025_12': {}}

# 1. Latih Holt-Winters pada seluruh data 2022-2025
hw_preds_dict = {}
for target in targets:
    hw_data = monthly_data[target].values
    try:
        # Kembali ke additive karena terbukti sangat stabil mencetak 3.6
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        # THE MAGIC TRICK: dropna() di sini akan OTOMATIS membuang seluruh tahun 2022
        # Karena Lag_12 untuk tahun 2022 adalah NaN. 
        # Akibatnya Model Linear HANYA belajar dari pola super bersih (Jan 2023 - pertengahan 2025)!
        ...
        # BLENDING: Model Linear sekarang punya insting tahunan (Lag 12), bobotnya dinaikkan!
        final_val = (0.45 * pred_bayes) + (0.30 * pred_ridge) + (0.25 * pred_hw)
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE 2023 CLEAN ISOLATION) ---")
...
submission_df.to_csv('submission_2023_isolation.csv', index=False)
print("\n[LOCKED] File 'submission_2023_isolation.csv' siap!")
print("Model Linear kini dilatih hanya dengan data paling matang (2023-2025) plus ingatan Lag 12. Gas pol!")