...
# Data murni Pasca-Covid (2022+)
monthly_data = monthly_data[monthly_data['Date'] >= '2022-01-01'].reset_index(drop=True)

# Billion Scaling (Mencegah error limitasi miliaran)
monthly_data['Total_Claim_B'] = monthly_data['Total_Claim'] / 1e9

# ==========================================
# 3. FITUR OTENTIK 3.6 (DIKEMBALIKAN)
# ==========================================
def create_features(df):
    df = df.copy()
    
    # Eskalator natural bulan
    df['Month'] = df['Date'].dt.month
    
    for col in ['Claim_Frequency', 'Total_Claim_B']:
        # Lag 6 dikembalikan! Ternyata ini sangat krusial untuk memori tengah tahun.
        for i in [1, 2, 3, 6]:
...
# ==========================================
# 4. THE MULTI-WINDOW ENSEMBLE (PEMECAH 3.6)
# ==========================================
print("\nMelatih Model Rekursif (The Multi-Window Apex)...")
current_ts_data = ts_data.copy()
final_preds = {'2025_08': {}, '2025_09': {}, '2025_10': {}, '2025_11': {}, '2025_12': {}}

# Latih Holt-Winters Damped (Mode Super Aman)
hw_preds_dict = {}
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        # DATASET 1: LONG-TERM (2022-2025)
        train_df_full = temp_df[temp_df['Date'] < pred_date].dropna()
        X_train_full = train_df_full[features]
        y_train_full = train_df_full[target]
        
        # DATASET 2: SHORT-TERM (2023-2025) HANYA UNTUK INFLASI TERBARU
        train_df_recent = train_df_full[train_df_full['Date'] >= '2023-01-01']
        X_train_recent = train_df_recent[features]
        y_train_recent = train_df_recent[target]
        
        X_test = temp_df[temp_df['Date'] == pred_date][features].fillna(0)
        
        # MODEL 1: BayesianRidge belajar dari seluruh sejarah (Stabilisator)
        bayes_full = make_pipeline(StandardScaler(), BayesianRidge()).fit(X_train_full, y_train_full)
        
        # MODEL 2: Ridge belajar HANYA dari sejarah terbaru (Agresor Inflasi)
        ridge_recent = make_pipeline(StandardScaler(), Ridge(alpha=1.0)).fit(X_train_recent, y_train_recent)
        
        pred_bayes = bayes_full.predict(X_test)[0]
        pred_ridge = ridge_recent.predict(X_test)[0]
        pred_hw = hw_preds_dict[target][i]
        
        # THE NEW APEX BLEND (Bebas kembar 3.64603 karena pembagian data yang dinamis)
        final_val = (0.35 * pred_bayes) + (0.35 * pred_ridge) + (0.30 * pred_hw)
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE MULTI-WINDOW APEX) ---")
...
submission_df.to_csv('submission_multi_window.csv', index=False)
print("\n[LOCKED] File 'submission_multi_window.csv' siap!")
print("Fitur 3.6 dikembalikan, Otak AI dibelah dua (Long-Term & Short-Term). Bismillah jebol < 3.0!")