...
# ==========================================
# 4. THE CUSTOM PROPHET ENSEMBLE
# ==========================================
print("\nMelatih Model Rekursif (The Custom Prophet)...")
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        ...
        # -------------------------------------------------------------------
        # ALGORITMA 1: CUSTOM BASELINE (Pengganti Holt-Winters yang suka error)
        # -------------------------------------------------------------------
        # A. Ekstrak Tren Utama dengan Huber (Kebal Outlier)
        ...
        # B. Ekstrak Rata-Rata Musiman (Additive)
        train_df['Seasonality_Value'] = train_df[target] - train_df['Trend_Base']
        seasonality_map = train_df.groupby('Month')['Seasonality_Value'].mean().to_dict()
        
        # C. Tebak Baseline Masa Depan
        ...
        future_seasonality = seasonality_map.get(future_month, 0)
        
        pred_baseline = future_trend + future_seasonality
        # -------------------------------------------------------------------
        ...
        # THE GRAND BLEND: 30% Tren Musiman Murni + 40% Bayes + 30% Ridge
        final_val = (0.30 * pred_baseline) + (0.40 * pred_bayes) + (0.30 * pred_ridge)
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE CUSTOM PROPHET) ---")
...
submission_df.to_csv('submission_custom_prophet.csv', index=False)
print("\n[LOCKED] File 'submission_custom_prophet.csv' siap!")
print("Dibuat dari nol. Bebas flatline XGBoost, bebas error Holt-Winters! BISMILLAH < 3.0!")