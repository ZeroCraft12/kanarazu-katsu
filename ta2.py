...
def create_features(df):
    ...
    for col in ['Log_Freq', 'Log_Total']:
        # HANYA gunakan lag pendek, JANGAN gunakan lag 12
        for i in [1, 2, 3, 6]:
...
# ==========================================
# 3. RECURSIVE ENSEMBLE TRAINING (UN-DAMPED)
# ==========================================
print("\nMelatih Model Recursive Autoregressive (The Un-Damped Revival)...")
current_ts_data = ts_data.copy()
final_preds = {'2025_08': {}, '2025_09': {}, '2025_10': {}, '2025_11': {}, '2025_12': {}}

# 1. Latih Holt-Winters SEKALI saja di awal (TANPA REM/DAMPED)
hw_preds_dict = {}
for target in targets:
    hw_data = monthly_data[target].values
    try:
        # DAMPED_TREND DIHAPUS: Biarkan AI mengikuti inflasi medis secara natural!
        hw_fit = ExponentialSmoothing(hw_data, trend='add', seasonal='add', seasonal_periods=12).fit(optimized=True)
        hw_preds_dict[target] = hw_fit.forecast(steps=5)
    except:
        hw_fit = ExponentialSmoothing(hw_data, trend='add').fit(optimized=True)
        hw_preds_dict[target] = hw_fit.forecast(steps=5)
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        ...
        # Pelatihan (Ridge dilonggarkan menjadi alpha=1.0 agar lebih responsif)
        ...
        # BLENDING BARU: Bobot HW dinaikkan agar tren inflasinya lebih dominan
        final_log = (0.40 * pred_bayes) + (0.20 * pred_ridge) + (0.40 * pred_hw)
...
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (THE UN-DAMPED PERFECTED REVIVAL) ---")
...
submission_df.to_csv('submission_the_revival2.csv', index=False)
print("\n[LOCKED] File 'submission_the_revival2.csv' siap!")
print("Rem tangan sudah dilepas, AI siap terbang mengejar target panitia. BISMILLAH < 3!")