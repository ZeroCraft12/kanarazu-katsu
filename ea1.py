...
# ==========================================
# 1. PONDASI DATA (ANTI-OUTLIER 98.0%)
# ==========================================
...
# ---------------------------------------------------------
# IBNR PATCH (Pengaman Data Bulan Terakhir)
# ---------------------------------------------------------
...
# THE GHOST MEMORY TRICK: 
# Muat dari 2021 sebagai "tumbal" untuk menghitung Lag 12 Bulan!
monthly_data = monthly_data[monthly_data['Date'] >= '2021-01-01'].reset_index(drop=True)

# THE BILLION SCALING: Tetap pertahankan skala raw (Skor 3.6 Pahlawan)
monthly_data['Total_Claim_B'] = monthly_data['Total_Claim'] / 1e9

# ==========================================
# 2. ADVANCED AUTOREGRESSIVE FEATURES
# ==========================================
def create_features(df):
    df = df.copy()
    df['Month'] = df['Date'].dt.month
    for col in ['Claim_Frequency', 'Total_Claim_B']:
        # KITA TAMBAHKAN LAG 12 KARENA SEKARANG KITA PUNYA DATA 2021
        for i in [1, 2, 3, 6, 12]:
            df[f'{col}_lag_{i}'] = df[col].shift(i)
            
        # Linear models love Roll Mean & EWMA
...
# ==========================================
# 3. RECURSIVE ENSEMBLE TRAINING (ISOLATED)
# ==========================================
print("\nMelatih Model Recursive (The Sub-3.0 Apex)...")
current_ts_data = ts_data.copy()
final_preds = {'2025_08': {}, '2025_09': {}, '2025_10': {}, '2025_11': {}, '2025_12': {}}

# 1. Latih Holt-Winters (ISOLASI DATA 2022 KE ATAS AGAR TREN COVID TIDAK MASUK)
hw_preds_dict = {}
for target in targets:
    # HW hanya boleh melihat data yang bersih (2022+)
    hw_data_clean = monthly_data[monthly_data['Date'] >= '2022-01-01'][target].values
    try:
        hw_fit = ExponentialSmoothing(hw_data_clean, trend='add', seasonal='add', seasonal_periods=12, damped_trend=True).fit(optimized=True)
        hw_preds_dict[target] = hw_fit.forecast(steps=5)
    except:
        hw_fit = ExponentialSmoothing(hw_data_clean, trend='add', damped_trend=True).fit(optimized=True)
        hw_preds_dict[target] = hw_fit.forecast(steps=5)
...
for i, pred_date in enumerate(months_to_predict):
    ...
    for target in targets:
        # ISOLASI DATA TRAINING: Model Linear hanya dilatih di data 2022 ke atas!
        # Dropna otomatis akan membuang baris yang Lag_12 nya kosong (yaitu tahun 2021)
        train_df = temp_df[(temp_df['Date'] < pred_date) & (temp_df['Date'] >= '2022-01-01')].dropna()
        X_train = train_df[features]
        y_train = train_df[target]
        
        X_test = temp_df[temp_df['Date'] == pred_date][features].fillna(0)
        
        # Penyesuaian alpha pada Ridge agar cocok dengan skala raw data
        ...
        # BLENDING ULTIMATE
        final_val = (0.45 * pred_bayes) + (0.25 * pred_ridge) + (0.30 * pred_hw)
...
submission_df.to_csv('submission_sub3_apex.csv', index=False)
print("\n[LOCKED] File 'submission_sub3_apex.csv' siap!")
print("Ini adalah batas absolut presisi matematis! Mari jemput skor kepala 2!")