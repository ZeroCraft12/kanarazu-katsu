...
# ==========================================
# 3. ACTUARIAL MACHINE LEARNING (POISSON & TWEEDIE)
# ==========================================
print("\nMelatih Model dengan Actuarial Objectives (Poisson & Tweedie)...")
...
for target in targets:
    for pred_date in months_to_predict:
        ...
        # ---------------------------------------------------------------
        # PERUBAHAN ALGORITMA:
        # 1. Frekuensi Orang Sakit -> Objective: Poisson
        # 2. Total Uang Klaim -> Objective: Tweedie (Variance Power 1.5)
        # ---------------------------------------------------------------
        if target == 'Claim_Frequency':
            m_lgb = lgb.LGBMRegressor(objective='poisson', learning_rate=0.04, max_depth=4, n_estimators=150, random_state=42, verbose=-1)
            m_xgb = xgb.XGBRegressor(objective='count:poisson', learning_rate=0.04, max_depth=4, n_estimators=150, random_state=42)
        else: # Total_Claim
            m_lgb = lgb.LGBMRegressor(objective='tweedie', tweedie_variance_power=1.5, learning_rate=0.04, max_depth=4, n_estimators=150, random_state=42, verbose=-1)
            m_xgb = xgb.XGBRegressor(objective='reg:tweedie', tweedie_variance_power=1.5, learning_rate=0.04, max_depth=4, n_estimators=150, random_state=42)
...
# ==========================================
# 4. EXPORT ACTUARIAL PREDICTION
# ==========================================
submission_rows = []
print("\n--- HASIL PREDIKSI (ACTUARIAL ML) ---")
print("Algoritma sekarang memahami bahwa data ini adalah data Asuransi, bukan data normal!")

for month_key in adv_preds.keys():
    final_freq = adv_preds[month_key]['Claim_Frequency']
    final_total = adv_preds[month_key]['Total_Claim']
    
    # Severity dihitung otomatis
...
submission_df.to_csv('submission_actuarial_tweedie.csv', index=False)
print("\nFile 'submission_actuarial_tweedie.csv' siap! Ini adalah standar Grandmaster untuk Data Asuransi.")